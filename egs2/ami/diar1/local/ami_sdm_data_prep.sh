...
#  for(i=6;i<=NF;i++) printf(" %s", $i); printf "\n"}' $SEGS | sort -u | uniq > $dir/text

# (1c) Make segment files from transcript
#segments file format is: utt-id side-id start-time end-time, e.g.:
#AMI_ES2011a_H00_FEE041_0003415_0003484 AMI_FEE041_ES2011a_H00__0003415_0003484
awk '{
       segment=$1;
       split(segment,S,"[_]");
...
#        segment=$1;
#        split(segment,S,"[_]");
#        audioname=S[1]"_"S[2]"_"S[3]"_"S[4]; startf=S[6]; endf=S[7];
#        print segment " " audioname " " startf/100 " " endf/100 " "
# }' < $dir/text > $dir/segments
...
sed -e 's?.*/??' -e 's?.wav??' $dir/wav.flist | \
 perl -ne 'split; $_ =~ m/(.*)\..*/; print "AMI_$1_SDM\n"' | \
  paste - $dir/wav.flist | sort -u > $dir/wav1.scp

#Keep only training part of waves
#awk '{print $2}' $dir/segments | sort -u | join - $dir/wav1.scp | sort -o $dir/wav2.scp
grep -f local/split_train.orig $dir/wav1.scp | sort -u > $dir/wav2.scp
...
# into shorter pieces].
# We use the fifth underscore-separated fields of the utterance-id
# as the speaker-id, e.g. 'AMI_EN2001a_SDM_FEO065_0090130_0090775' becomes 'FEO065'.
