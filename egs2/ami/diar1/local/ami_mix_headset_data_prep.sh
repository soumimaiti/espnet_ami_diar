...
if [ $# -ne 2 ] || [ "$2" != "mix_headset" ]; then
  echo "Usage: $0 /path/to/AMI mix_headset"
  echo "e.g. $0 /foo/bar/AMI mix_headset"
  echo "note: the 2nd 'mix_headset' argument is for compatibility with other scripts."
  exit 1;
fi
...
dir=data/local/mix_headset/train
odir=data/mix_headset/train_orig
mkdir -p $dir
...
find $AMI_DIR -iname '*.Mix-Headset*.wav' | sort > $dir/wav.flist
...
sed -e 's?.*/??' -e 's?.wav??' $dir/wav.flist | \
 perl -ne 'split; $_ =~ m/(.*)\..*/; print "AMI_$1\n"' | \
...
echo AMI mix_headset data preparation succeeded.
